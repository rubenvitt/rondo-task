...
export default function TaskListItem({item}: Props) {
    ...
    let {mutate} = useMutation(async (state: boolean) => await toggleItem(item, state), {
        onSuccess: async () => {
    ...
    return <li className="py-4">
        <div className="relative flex items-start">
            ...
            <div className="ml-3 text-sm leading-6">
                <label htmlFor="comments" className={classNames(checked ? 'text-gray-500' : "font-medium text-gray-900", "transition")}>
                    {item.label}
                </label>
